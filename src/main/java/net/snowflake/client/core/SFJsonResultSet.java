...
public abstract class SFJsonResultSet extends SFBaseResultSet {
  ...
  public Date getDate(int columnIndex, TimeZone tz) throws SFException {
    ...
    if (Types.TIMESTAMP == columnType) {
      ...
      return new SnowflakeDateWithTimezone(getTimestamp(columnIndex, tz).getTime(), dateFormatter);

    } else if (Types.DATE == columnType) {
      if (tz == null || !resultSetSerializable.getFormatDateWithTimeZone()) {
        return new SnowflakeDateWithTimezone(
            ArrowResultUtil.getDate(Integer.parseInt((String) obj)), dateFormatter);
      }
      return new SnowflakeDateWithTimezone(
          ArrowResultUtil.getDate(Integer.parseInt((String) obj), tz, sessionTimeZone),
          dateFormatter);
    }
