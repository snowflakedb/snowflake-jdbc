...
public abstract class SFJsonResultSet extends SFBaseResultSet
{
  ...
  public String getString(int columnIndex) throws SFException
  {
    ...
    switch (columnType)
    {
      ...
      case Types.DATE:
        ...
        logger.debug("Converting date to string from: {} to: {}", obj, dateStr);
      ...
      case Types.TIME:
        ...
        logger.debug("Converting time to string from: {} to: {}", obj, timeStr);
  ...
  public long getLong(int columnIndex) throws SFException
  {
    ...
    catch (NumberFormatException nfe)
    {
      ...
      {
        throw new SFException(ErrorCode.INVALID_VALUE_CONVERT, columnType, SnowflakeUtil.LONG_STR, obj);
      }
    }
  }
  ...
  public byte[] getBytes(int columnIndex) throws SFException
  {
    ...
    try
    {
      ...
      // for different types.
      // The private build jar doesn't support FLOAT/DOUBLE to byte array too, because the fix depends on ARROW
      // format, which was not supported in the older version.
      switch (columnType)
      {
        case Types.NUMERIC:
        case Types.INTEGER:
