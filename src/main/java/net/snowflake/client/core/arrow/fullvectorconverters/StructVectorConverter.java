...
public class StructVectorConverter implements ArrowFullVectorConverter {
    ...
    public FieldVector convert() throws SFException, SnowflakeSQLException {
        ...
        for (FieldVector childVector : childVectors) {
            ...
            convertedVectors.add(ArrowFullVectorConverter.convert(allocator, childVector, context, session, idx, targetType));
        }

        List<Field> convertedFields = convertedVectors.stream().map(convertedVector -> convertedVector.getField()).collect(Collectors.toList());
        ...
        for (FieldVector convertedVector : convertedVectors) {
            TransferPair transferPair = convertedVector.makeTransferPair(converted.getChild(convertedVector.getName()));
