...
public class FixedSizeListVectorConverter implements ArrowFullVectorConverter{
    ...
    public FieldVector convert() throws SFException, SnowflakeSQLException {
        ...
        FieldVector convertedDataVector = ArrowFullVectorConverter.convert(allocator, dataVector, context, session, 0, valueTargetType);
        FixedSizeListVector convertedListVector = FixedSizeListVector.empty(listVector.getName(), listVector.getListSize(), allocator);
        ...
        convertedListVector.getValidityBuffer().setBytes(0L, validityBuffer, 0L, validityBuffer.capacity());
