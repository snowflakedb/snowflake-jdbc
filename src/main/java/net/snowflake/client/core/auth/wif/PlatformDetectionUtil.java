...
/**
 * Utility class for platform detection and Workload Identity Federation (WIF) operations.
 * This class provides:
 * - HTTP request execution for metadata services
 * - AWS credentials validation
 * - Constants for metadata service endpoints (used by other WIF components)
 */
...
public class PlatformDetectionUtil {
    ...
    private static final List<Pattern> VALID_WIF_ARN_PATTERNS = Arrays.asList(
            Pattern.compile("^arn:[^:]+:iam::[^:]+:user/.+$"),
            Pattern.compile("^arn:[^:]+:sts::[^:]+:assumed-role/.+$")
    );

    public static String performPlatformDetectionRequest(HttpRequestBase httpRequest, double timeoutSeconds)
    ...
    public static boolean isValidArnForWif(String arn) {
        ...
        return VALID_WIF_ARN_PATTERNS.stream()
                .anyMatch(pattern -> pattern.matcher(arn).matches());
    }
