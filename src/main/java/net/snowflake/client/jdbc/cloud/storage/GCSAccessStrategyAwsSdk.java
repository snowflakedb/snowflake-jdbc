...
class GCSAccessStrategyAwsSdk implements GCSAccessStrategy {
  ...
  private final S3AsyncClient transferManagerClient;
  ...
  GCSAccessStrategyAwsSdk(StageInfo stage, SFBaseSession session) throws SnowflakeSQLException {
    ...
    overrideConfiguration.retryStrategy(
        DefaultRetryStrategy.doNotRetry()); // we handle retries ourselves
    clientBuilder.overrideConfiguration(overrideConfiguration.build());
    transferManagerClient = clientBuilder.build();
  }
  ...
  public Map<String, String> download(
    ...
    try (S3TransferManager tx =
        S3TransferManager.builder()
            .s3Client(transferManagerClient)
  ...
  public void shutdown() {
    ...
    if (this.transferManagerClient != null) {
      this.transferManagerClient.close();
    }
  }
}
