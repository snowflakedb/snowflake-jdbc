...
public class StructuredTypesGetStringArrowJsonCompatibilityIT
  ...
  public void testRunAsGetString(ResultSetFormatType queryResultFormat, String selectSql, String expectedStructureTypeRepresentation) throws SQLException {
    withFirstRow(
  ...
  public static class SampleProvider extends SnowflakeArgumentsProvider {
    ...
    protected List<Arguments> rawArguments(ExtensionContext context) {
      ...
      samples.add(Arguments.of(
              ...
              "[{\"string\":\"a\"}, {\"string\":\"b\"}]"));
      samples.add(Arguments.of("select {'string':'a'}::object(string varchar)", "{\"string\":\"a\"}"));
      samples.add(Arguments.of(
              "select {'x':'a','b':'a','c':'a','d':'a','e':'a'}::object(x varchar,b varchar,c varchar,d varchar,e varchar)",
              "{\"x\":\"a\",\"b\":\"a\",\"c\":\"a\",\"d\":\"a\",\"e\":\"a\"}"));
      samples.add(Arguments.of("select {'string':[1,2,3]}::object(string array(int))", "{\"string\":[1,2,3]}"));
      samples.add(Arguments.of(
      ...
      samples.add(Arguments.of("select {'time': '22:14:55'::TIME}::object(time TIME)", "{\"time\":\"22:14:55\"}"));
