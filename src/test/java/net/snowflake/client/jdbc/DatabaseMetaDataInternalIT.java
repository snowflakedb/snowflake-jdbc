...
//    st.execute("create or replace table JDBC_TBL211(colA string(25) NOT NULL DEFAULT 'defstring')");
  ...
  public void testGetColumn() throws SQLException
  {
    ...
    assertEquals(getAllObjectCountInDBViaInforSchema(null, getAllColumsCount), getSizeOfResultSet(resultSet));
    ...
    assertEquals(1, getSizeOfResultSet(resultSet) + 1); // read the rest of rows
  ...
  public void testGetSchema() throws SQLException
  {
    ...
    assertEquals(getAllObjectCountInDBViaInforSchema(null, getSchemaCount), getSizeOfResultSet(resultSet));
   ...
   */
  @Test
  public void testGetTablesReusingCachedResults() throws Throwable
  {
  ...
  {
    // Note: currently it is not reliable to use information_schema to check temporary tables
    String getAllTable = "select count(*) from db.information_schema."
                         + "tables where table_type = 'BASE TABLE' and not is_transient";
    String getAllTransient = "select count(*) from db.information_schema."
                             + "tables where table_type = 'BASE TABLE' and is_transient";
    String getAllTemporary = "select count(*) from db.information_schema."
                             + "tables where table_type like '%TEMPORARY%'";
    ...
    stmt.execute("create or replace schema JDBC_DB1.JDBC_SCHEMA22");
    stmt.execute("create or replace table JDBC_TABLE(colA string, colB decimal, colC timestamp)");
    stmt.execute("create or replace temporary table JDBC_TEMP(colA string, colB decimal, colC timestamp)");
    stmt.execute("create or replace Transient table JDBC_TRAN(colA string, colB decimal, colC timestamp)");
    resultSet = databaseMetaData.getTables("JDBC_DB1", "JDBC_SCHEMA22", null, new String[]{"TABLE"});
    assertEquals(1, getSizeOfResultSet(resultSet));

    resultSet = databaseMetaData.getTables("JDBC_DB1", "JDBC_SCHEMA22", null, new String[]{"TEMPORARY"});
    assertEquals(1, getSizeOfResultSet(resultSet));

    resultSet = databaseMetaData.getTables("JDBC_DB1", "JDBC_SCHEMA22", null, new String[]{"TRANSIENT"});
    assertEquals(1, getSizeOfResultSet(resultSet));

    resultSet =
        databaseMetaData.getTables("JDBC_DB1", "JDBC_SCHEMA22", null, new String[]{"TEMPORARY", "TABLE", "TRANSIENT"});
    assertEquals(3, getSizeOfResultSet(resultSet));

    int allTableCnt = getAllObjectCountInDBViaInforSchema(stmt, getAllTable);
    int allViewCnt = getAllObjectCountInDBViaInforSchema(stmt, getAllView);
    int allTableAndViewCnt = allViewCnt + allTableCnt;
    int allTransientCnt = getAllObjectCountInDBViaInforSchema(stmt, getAllTransient);
    int allTemporaryCnt = getAllObjectCountInDBViaInforSchema(stmt, getAllTemporary);
    ...
    assertEquals(allViewCnt, getSizeOfResultSet(resultSet));

    resultSet = databaseMetaData.getTables(null, null, null, new String[]{"TABLE"});
    assertEquals(allTableCnt, getSizeOfResultSet(resultSet));

    resultSet = databaseMetaData.getTables(null, null, null, new String[]{"VIEW"});
    assertEquals(allViewCnt, getSizeOfResultSet(resultSet));


    resultSet = databaseMetaData.getTables(null, null, null, new String[]{"TRANSIENT"});
    assertEquals(allTransientCnt, getSizeOfResultSet(resultSet));

    resultSet = databaseMetaData.getTables(null, null, null, new String[]{"TEMPORARY"});
    assertEquals(allTemporaryCnt, getSizeOfResultSet(resultSet));

    resultSet = databaseMetaData.getTables(null, null, null, new String[]{"TABLE", "TRANSIENT"});
    assertEquals(allTableCnt + allTransientCnt, getSizeOfResultSet(resultSet));

    resultSet = databaseMetaData.getTables(null, null, null, new String[]{"TABLE", "TEMPORARY"});
    assertEquals(allTableCnt + allTemporaryCnt, getSizeOfResultSet(resultSet));

    resultSet = databaseMetaData.getTables(null, null, null, new String[]{"TABLE", "TEMPORARY", "TRANSIENT"});
    assertEquals(allTableCnt + allTemporaryCnt + allTransientCnt, getSizeOfResultSet(resultSet));

    resultSet = databaseMetaData.getTables(null, null, null, new String[]{"TABLE", "TEMPORARY", "TRANSIENT", "VIEW"});
    assertEquals(allTemporaryCnt + allTransientCnt + allTableAndViewCnt, getSizeOfResultSet(resultSet));
    ...
    assertEquals(allTableAndViewCnt, getSizeOfResultSet(resultSet));
    ...
    resultSet.next();
    assertEquals("TEMPORARY", resultSet.getString(1));
    resultSet.next();
    assertEquals("TRANSIENT", resultSet.getString(1));
    resultSet.next();
  ...
  private int getAllObjectCountInDBViaInforSchema(Statement st, String SQLCmdTemplate) throws SQLException
  {
    int objectCount = 0;
    Connection con = getConnection();
    if (st == null)
