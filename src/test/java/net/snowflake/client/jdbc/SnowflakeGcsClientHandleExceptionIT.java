...
public class SnowflakeGcsClientHandleExceptionIT extends AbstractDriverIT {
    ...
    public void setup() throws SQLException {
        ...
        SnowflakeGCSClient client = SnowflakeGCSClient.createSnowflakeGCSClient(agent.getStageInfo(), agent.getEncryptionMaterial().get(0), sfSession);
    ...
    public void error401RenewExpired() throws SQLException, InterruptedException {
        ...
        spyingClient.handleStorageException(new StorageException(401, "Unauthenticated"), 0, "upload", sfSession,
                command);
        ...
        spyingClient.handleStorageException(new StorageException(401, "Unauthenticated"), 0, "upload", null, command);
        Mockito.verify(spyingClient, Mockito.times(1)).renew(Mockito.anyMap());
        spyingClient.handleStorageException(new StorageException(401, "Unauthenticated"), 0, "upload", sfSession, null);
        Mockito.verify(spyingClient, Mockito.times(1)).renew(Mockito.anyMap());
        ...
        Thread thread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    spyingClient.handleStorageException(new StorageException(401, "Unauthenticated"), maxRetry, "upload", sfSession,
                            command);
    ...
    public void error401OverMaxRetryThrow() throws SQLException {
        spyingClient.handleStorageException(new StorageException(401, "Unauthenticated"), overMaxRetry, "upload",
                sfSession, command);
    }
    ...
    public void errorInvalidKey() throws SQLException {
        // Unauthenticated, renew is called.
        spyingClient.handleStorageException(new Exception(new InvalidKeyException()), 0, "upload", sfSession, command);
    }
    ...
    public void errorInterruptedException() throws SQLException {
        ...
        try {
            spyingClient.handleStorageException(new InterruptedException(), 0, "upload", sfSession, command);
        } catch (Exception e) {
            Assert.fail("Should not have exception here");
        }
        Mockito.verify(spyingClient, Mockito.never()).renew(Mockito.anyMap());
        spyingClient.handleStorageException(new InterruptedException(), 26, "upload", sfSession, command);
    }
    ...
    public void errorSocketTimeoutException() throws SQLException {
        // Can still retry, no error thrown
        try {
            spyingClient.handleStorageException(new SocketTimeoutException(), 0, "upload", sfSession, command);
        } catch (Exception e) {
            Assert.fail("Should not have exception here");
        }
        Mockito.verify(spyingClient, Mockito.never()).renew(Mockito.anyMap());
        spyingClient.handleStorageException(new SocketTimeoutException(), 26, "upload", sfSession, command);
    }
