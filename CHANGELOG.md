...
- v4.3.5-SNAPSHOT
  ...
  - Deprecated the `MIN_TLS_VERSION` and `MAX_TLS_VERSION` connection properties in favour of the JVM-wide `jdk.tls.client.protocols` system property, which now takes precedence and, unlike the connection properties, also constrains PUT/GET stage transfers; the connection properties are still honoured when the system property is absent. The versions named by the system property are intersected with the TLS 1.2-1.3 range the driver supports, so it can tighten the handshake but cannot lower the driver's TLS 1.2 floor (snowflakedb/snowflake-jdbc#10).
  - Fixed the TLS version settings being held in process-global static fields and omitted from the HTTP client cache key, so a connection could silently inherit another connection's enabled TLS protocols; they are now part of `HttpClientSettingsKey` and scoped per connection. As a consequence they no longer reach HTTP clients built outside a session (Workload Identity platform detection, the sessionless `SnowflakeFileTransfer` entry point, and result sets rebuilt from a serialized form), which use the driver defaults of TLS 1.2-1.3; use `jdk.tls.client.protocols` to constrain those paths (snowflakedb/snowflake-jdbc#10).
  - Fixed invalid `MIN_TLS_VERSION`/`MAX_TLS_VERSION` values and an inverted version range surfacing as an unwrapped `IllegalArgumentException` from socket factory construction instead of a `SnowflakeSQLException` when the property is set, and fixed TLS version availability being resolved from the protocols the JSSE provider implements rather than the ones it will actually enable, so a version banned through the `jdk.tls.disabledAlgorithms` security property could still be offered and then never negotiated, surfacing as an opaque handshake failure (snowflakedb/snowflake-jdbc#10).
  - Fixed cipher suite selection using `SSLServerSocketFactory`'s server-side default suites for the driver's client connections, which both discarded the JVM-wide `jdk.tls.client.cipherSuites` and `jdk.tls.disabledAlgorithms` settings and bypassed Apache HttpClient's weak-suite filtering. Selection is now left to JSSE unless `https.cipherSuites` is set, whose value is also trimmed. This covers the Snowflake connection and result-set chunk downloads only; PUT/GET stage transfers go through cloud storage SDKs that supply their own cipher list, where `jdk.tls.disabledAlgorithms` is the only effective control (snowflakedb/snowflake-jdbc#14).
  - Fixed GET from a client-side-encrypted (internal/temporary) GCS stage writing the still-encrypted bytes to the destination and reporting the transfer as successful, on the `GCSAccessStrategyAwsSdk` strategy (`useVirtualUrl`, selected by `SNOWFLAKE_GCS_FORCE_VIRTUAL_STYLE_DOMAINS` on the downscoped-token path); the default GCP path (`GCSDefaultAccessStrategy`) is unaffected. GCS returns the object's user metadata as `x-goog-meta-*` response headers, which the AWS S3 response model never maps into `HeadObjectResponse.metadata()` because it populates that map from `x-amz-meta-*` only, so the encryption material was read from a permanently empty map and decryption was skipped. The metadata is now read from the raw response headers. `getFile`/`downloadStream` on the same path failed with `File metadata incomplete` for the same reason and now succeed.
