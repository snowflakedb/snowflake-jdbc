...
# Changelog
- v4.3.5-SNAPSHOT
  - Removed the unused Conscrypt library (`org.conscrypt:conscrypt-openjdk-uber`) from the self-contained (fat) and FIPS JARs. It was only transitive weight from `google-cloud-storage` and is never used by the driver (snowflakedb/snowflake-jdbc#XXXX).
