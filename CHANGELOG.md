...
- v4.3.2-SNAPSHOT
  ...
  - Fixed flaky `DatabaseMetaDataIT.testGetPrimarykeys`, `SnowflakeDriverIT.testConstraints`, and `DatabaseMetaDataLatestIT.test[No]PatternSearchAllowedForPrimaryAndForeignKeys` (eventually-consistent PK/FK constraint metadata after DDL) by retrying the constraint metadata lookups until visible, and added `DatabaseMetadataWiremockLatestIT` which deterministically validates the `getTables`/`getPrimaryKeys`/`getImportedKeys` SHOW-result mapping using stubbed responses (no live account or metadata-propagation dependency). Test-only change with no driver behavior impact (snowflakedb/snowflake-jdbc#2673).
