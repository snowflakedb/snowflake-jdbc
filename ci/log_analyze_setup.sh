...
function setup_log_env() {
    ...
    CLIENT_KNOWN_SMM_FILE_PATH=$CLIENT_KNOWN_SSM_FILE_PATH
    if ["$WORKSPACE" == "/mnt/workspace"]; then
        ...
        CLIENT_KNOWN_SMM_FILE_PATH = $CLIENT_KNOWN_SSM_FILE_PATH_DOCKER
    fi
    echo "[INFO] CLIENT_LOG_DIR_PATH=$CLIENT_LOG_DIR_PATH"  
    echo "[INFO] CLIENT_LOG_FILE_PATH=$CLIENT_LOG_FILE_PATH"
    echo "[INFO] CLIENT_KNOWN_SSM_FILE_PATH=$CLIENT_KNOWN_SMM_FILE_PATH"
    ...
    if [[ -f $CLIENT_KNOWN_SMM_FILE_PATH ]]; then
        rm -f $CLIENT_KNOWN_SMM_FILE_PATH
    fi

    touch $CLIENT_KNOWN_SMM_FILE_PATH
